...
function _updateUserUI() {
  ...
  if (navKpi) navKpi.style.display = ['soongeun', 'admin', 'jaehyun', 'yoonhee', 'younghyun'].includes(currentUser.id) ? '' : 'none';
}
// ══════════════════════════════════════════
...
function renderKpiGrandTable() {
  ...
  el.innerHTML = `<div style="overflow-x:auto;"><table class="kpi-tbl" style="width:max-content;table-layout:fixed;">
    ...
    <tbody>
      ...
      <tr><td style="${tdL}">광고주별 KPI 달성률(평균)</td><td style="${tdAC}font-weight:600;">${totalAdvAvg == null ? nd : totalAdvAvg + '%'}</td>${cols.map(advRateCell).join('')}</tr>
