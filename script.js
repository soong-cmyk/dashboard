...
// ══════════════════════════════════════════
const USERS = [];
// 직급 레벨: 숫자가 낮을수록 높은 직급 (1=대표이사 ~ 6=일반)
...
function enterUserEdit(uid) {
  ...
  // 직급 셀 → select
  const rankOpts = ['이사','본부장','실장','팀장','일반'].map(r =>
...
const FIELD_LABELS = {
  name:'광고내용', promo:'프로모션',
  cat:'카테고리', date:'발송일시', dateEnd:'종료일', media:'매체사',
  product:'상품', contract:'계약주체', seller:'매출처', adv:'광고주', agency:'대행사', content:'브랜드',
  ops:'담당자', dept:'부서', status:'단계',
  qty:'발송예약수량', svc:'서비스물량', unit:'단가', sellUnit:'매출단가', buyUnit:'매입단가', disc:'할인금액',
  comm:'수수료율', agrate:'대행료율', target:'타겟조건', dtarget:'디타겟조건', msg:'발송문구', msgFinal:'검수완료문구',
  note:'특기사항', testOk:'테스트수신', sent:'성과입력대기',
  actual:'실발송수량', clicks:'클릭수', ctr:'클릭률', db:'DB등록수', dbr:'DB등록률',
  billBase:'정산기준', sellBillBase:'매출정산기준', buyBillBase:'매입정산기준',
  svcApplyAdv:'서비스수량 적용(광고주)', svcApplyMedia:'서비스수량 적용(매체)',
  adcostFixed:'광고비 수동입력', amtFixed:'실청구 수동입력', buyAmtFixed:'매입액 수동입력',
  revFixed:'매출수익 수동입력', profitFixed:'이익 수동입력',
  daAdcost:'DA광고비', daBillBase:'DA정산기준', daFeeYn:'DA수수료포함여부',
  daImp:'노출수', daClick:'클릭수(2차)', daConv:'전환수', daRev:'매출(DA)', daImage:'이미지',
  cpaFeeYn:'CPA수수료포함여부',
  cpsFinalSales:'최종정산매출', cpsTotalComm:'총수수료', cpsMediaComm:'매체수수료', stlMonth:'정산월',
  pcAdvUnit:'광고주단가', pcOhcCost:'OHC비용', pcDnuUnit:'DNU단가', pcInflow:'매체유입', pcAgree:'동의건',
...
function openDetail(idx, skipPush) {
  ...
  const stl  = _stlAmt(c);
  const adc  = stl.adc, real = stl.amt, buyAmt = stl.buyAmt, agf = stl.agFee, prf = stl.prf;
  const rev  = real - buyAmt;                       // 매출수익
  ...
  document.getElementById('dSellUnit').textContent  = unit    ? _fmtMoney(unit)    + '원'  : '—';
  document.getElementById('dBuyUnit').textContent   = buyUnit ? _fmtMoney(buyUnit) + '원'  : '—';
  const _showVal    = (val, suffix) => (val != null && val !== '') ? _fmtMoney(val) + suffix : '—';
  const _hasBillData = qty > 0 || actual > 0;
  const dBuyAmtEl = document.getElementById('dBuyAmt');
  if (dBuyAmtEl) dBuyAmtEl.textContent = (_hasBillData || c.buyAmtFixed != null) ? _showVal(buyAmt, '원') : '—';
  ...
  document.getElementById('dAdcost').textContent = (_hasBillData || c.adcostFixed != null) ? _showVal(adc, '원') : '—';
  document.getElementById('dDisc').textContent   = disc ? _fmtMoney(disc) + '원' : '—';
  document.getElementById('dAmt').textContent    = (_hasBillData || c.amtFixed != null)    ? _showVal(real, '원') : '—';
  document.getElementById('dComm').textContent   = comm ? comm + '%'            : '—';
  document.getElementById('dRev').textContent    = (_hasBillData || c.revFixed != null)    ? _showVal(rev, '원') : '—';
  document.getElementById('dAgrateLbl').textContent = `– 대행료 (${agrate}%)`;
  document.getElementById('dAgfee').textContent  = agf  ? _fmtMoney(agf)  + '원'  : '—';
  document.getElementById('dProfit').textContent = (_hasBillData || c.profitFixed != null) ? _showVal(prf, '원') : '—';
  ...
  if (dCardPerf) dCardPerf.style.display = (isPCcamp || isCPScamp) ? 'none' : '';
  // perfStatus 2차 입력 현황 반영
  document.getElementById('perfStatus').style.display   = (isDAcamp || isPCcamp || isCPScamp) ? 'none' : '';
  document.getElementById('perfStatusDA').style.display = isDAcamp ? '' : 'none';

  // DA / PC / CPS / CPA / 일반 카드 토글
  document.getElementById('dCardQty').style.display    = (isDAcamp || isPCcamp || isCPScamp) ? 'none' : '';
  ...
  if (isDAcamp) {
    ...
    setText('dDaAdcostShow', daAdc   ? _fmtMoney(daAdc)  + '원' : '—');
    setText('dDaBillBase',   c.daBillBase || '—');
    setText('dDaFeeYn',      c.daFeeYn    || '—');
    setText('dDaComm',       daComm  ? daComm + '%'            : '—');
    setText('dDaBuyUnit',    daBuyU  ? _fmtMoney(daBuyU) + '원' : '—');
    setText('dDaRev',        daRev   ? _fmtMoney(daRev)  + '원' : '—');
    setText('dDaProfit',     daAdc   ? _fmtMoney(daPrf)  + '원' : '—');
    const agLbl = document.getElementById('dDaAgratelbl');
    if (agLbl) agLbl.textContent = `– 대행료 (${daAgrate}%)`;
    setText('dDaAgfee',      daAgf   ? _fmtMoney(daAgf)  + '원' : '—');
    // 타겟 조건
  ...
  if (isDAcamp) {
    ...
    const daFields = [
      ...
      { id:'dDaRev',   val: rev,   fmt: v => _fmtMoney(Number(v))+'원', auto: false },
  ...
  if (isPCcamp) {
    ...
    setText('dPcCvr',     cvr);
    setText('dPcAdvUnit', advU    ? _fmtMoney(advU)    + '원' : nd);
    setText('dPcAdc',     adc     ? _fmtMoney(adc)     + '원' : nd);
    setText('dPcOhcCost', ohcCost ? _fmtMoney(ohcCost) + '원' : nd);
    setText('dPcDnuCost', dnuCost ? _fmtMoney(dnuCost) + '원' : nd);
    setText('dPcProfit',  profit  ? _fmtMoney(profit)  + '원' : nd);
  ...
  if (isCPScamp) {
    ...
    setText('dCpsFinalSales', finalSales ? _fmtMoney(finalSales) + '원' : nd);
    setText('dCpsTotalComm',  totalComm  ? _fmtMoney(totalComm)  + '원' : nd);
    setText('dCpsCommRate1',  commRate1);
    setText('dCpsMediaRate',  cpsRate ? cpsRate + '%' : nd);
    setText('dCpsMediaComm',  mediaComm  ? _fmtMoney(mediaComm)  + '원' : nd);
    setText('dCpsCommRate2',  commRate2);
    setText('dCpsBcRate',     bcRate);
    setText('dCpsBcProfit',   bcProfit   ? _fmtMoney(bcProfit)   + '원' : nd);
...
function _comboKeyNav(event, listId) {
  ...
  // .combo-add("+ 신규 추가" 등)도 같이 순회 대상에 넣어서 방향키/Tab/Enter로 고를 수 있게 한다.
...
function _calcCPSCore(prefix, fromFinalSales = false) {
  const p = prefix;
  const mp = p === 'r' ? 'r' : 'e';
  const finalSales = +document.getElementById(`${p}_cps_final_sales`)?.value || 0;
  const mediaName  = document.getElementById(`${mp}_media`)?.value  || '';
  const sellerName = document.getElementById(`${mp}_seller`)?.value || '';
  const isNaver    = sellerName === '네이버';
  const cpsRate    = _getCpsRate(mediaName, sellerName);

  const tcEl  = document.getElementById(`${p}_cps_total_comm`);
  const mcEl  = document.getElementById(`${p}_cps_media_comm`);
  const bcpEl = document.getElementById(`${p}_cps_bc_profit`);

  // 네이버: finalSales 변경 시 → 수동 플래그 초기화 후 재계산
  // 네이버: 수동 수정된 필드는 dataset.manual='1' 로 보호
  let totalComm, mediaComm, bcProfit;

  if (isNaver && (fromFinalSales || !tcEl?.dataset.manual)) {
    totalComm = finalSales ? Math.round(finalSales * 0.018) : 0;
    if (tcEl) { tcEl.value = totalComm || ''; if (fromFinalSales) delete tcEl.dataset.manual; }
  } else {
    totalComm = +tcEl?.value || 0;
  }

  if (isNaver && (fromFinalSales || !mcEl?.dataset.manual)) {
    mediaComm = finalSales ? Math.round(finalSales * cpsRate / 100) : 0;
    if (mcEl) { mcEl.value = mediaComm || ''; if (fromFinalSales) delete mcEl.dataset.manual; }
  } else {
    mediaComm = +mcEl?.value || 0;
  }

  if (fromFinalSales || !bcpEl?.dataset.manual) {
    bcProfit = totalComm - mediaComm;
    if (bcpEl) { bcpEl.value = bcProfit || ''; if (fromFinalSales) delete bcpEl.dataset.manual; }
  } else {
    bcProfit = +bcpEl?.value || 0;
  }

  // 라벨·힌트 업데이트
  const lblEl = document.getElementById(`${p}_cps_final_sales_lbl`);
  if (lblEl) lblEl.textContent = isNaver ? '유효결제금액' : '최종정산매출';
  const tcHint = document.getElementById(`${p}_cps_total_comm_hint`);
  if (tcHint) tcHint.textContent = isNaver ? '유효결제금액 × 1.8%' : '직접 입력';
  ...
  const _set = (id, val) => { const el = document.getElementById(id); if (el) el.value = val; };
  _set(`${p}_cps_media_rate`,  cpsRate ? cpsRate + '%' : '');
  _set(`${p}_cps_comm_rate1`,  finalSales > 0 ? commRate1.toFixed(2) + '%' : '');
  _set(`${p}_cps_comm_rate2`,  finalSales > 0 ? commRate2.toFixed(2) + '%' : '');
  _set(`${p}_cps_bc_rate`,     cpsRate ? bcRateVal.toFixed(2) + '%' : '');
  _set(`${p}_cps_profit_rate`, finalSales > 0 ? profitRate.toFixed(2) + '%' : '');
}

function calcCPS(fromFinalSales)     { _calcCPSCore('r', fromFinalSales); }
function calcCPSEdit(fromFinalSales) { _calcCPSCore('e', fromFinalSales); }
...
function submitReg() {
  ...
  DATA.unshift({
    ...
    profitFixed:  isCPA
      ...
      : isCPS
      ? ((el => (el?.dataset.manual && el.value !== '') ? +el.value : null)(document.getElementById('r_cps_bc_profit')))
      : (!isDA && !isPC ? ((el => (el?.dataset.manual && el.value !== '') ? +el.value : null)(document.getElementById('r_profit'))) : null),
    ...
    dateEnd:   (isDA || isCPA) ? `${document.getElementById('r_da_eyear').value}-${document.getElementById('r_da_emonth').value}-${document.getElementById('r_da_eday').value}` : '',
...
function resetRegForm() {
  ...
  ['r_cps_final_sales','r_cps_total_comm','r_cps_media_comm',
   ...
   'r_cps_exec_year','r_cps_exec_month','r_cps_stl_year','r_cps_stl_month']
    .forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
  ['r_cps_total_comm','r_cps_media_comm','r_cps_bc_profit'].forEach(id => {
    const el = document.getElementById(id); if (el) { el.readOnly = false; delete el.dataset.manual; }
  });
  _daImagesBase64 = [];
...
function openEdit() {
  ...
  const eCardPC   = document.getElementById('e_card_pc');
  const eCardCPA  = document.getElementById('e_card_cpa');
  const eCardCPS2 = document.getElementById('e_card_cps');
  if (eMsgSec)   eMsgSec.style.display   = (isEditDA || isEditPC || isEditCPA || isEditCPS) ? 'none' : '';
  ...
  if (isEditCPA) {
    ...
    _restoreCpaManual('e_cpa_adcost',  c.adcostFixed);
    const calcBuyUnit = Math.round((c.sellUnit || 0) * (1 - (c.comm || 0) / 100));
    _restoreCpaManual('e_cpa_buyUnit', c.buyUnit !== calcBuyUnit ? c.buyUnit : null);
    _restoreCpaManual('e_cpa_buyAmt',  c.buyAmtFixed);
    _restoreCpaManual('e_cpa_rev',     c.revFixed);
    _restoreCpaManual('e_cpa_profit',  c.profitFixed);
    calcCPAEdit();
  }
  if (isEditCPS) {
    ...
    ['e_cps_total_comm','e_cps_media_comm'].forEach(id => {
      const el = document.getElementById(id); if (el) delete el.dataset.manual;
    });
    // 브레인큐브 수익(원) 수기입력 고정값 복원 (있으면 manual 표시, 없으면 자동계산)
    const eCpsBcProfitRestoreEl = document.getElementById('e_cps_bc_profit');
    if (eCpsBcProfitRestoreEl) {
      if (c.profitFixed != null) { eCpsBcProfitRestoreEl.value = c.profitFixed; eCpsBcProfitRestoreEl.dataset.manual = '1'; }
      else { eCpsBcProfitRestoreEl.value = ''; delete eCpsBcProfitRestoreEl.dataset.manual; }
    }
    calcCPSEdit();
  }
...
function submitEdit() {
  ...
  const _histSkip = new Set(['id','regDate','regUser']);
  const _histVal = (v) => Array.isArray(v) ? (v.length ? `이미지 ${v.length}장` : '') : String(v ?? '');
  const snap = {};
  Object.keys(c).forEach(k => { if (!_histSkip.has(k)) snap[k] = _histVal(c[k]); });
  ...
  if (isEditDA) {
    ...
    c.date    = `${yr}-${mo}-${day} 00:00`;
    const eyr = document.getElementById('e_da_eyear')?.value || yr;
    c.dateEnd = `${eyr}-${document.getElementById('e_da_emonth')?.value || mo}-${document.getElementById('e_da_eday')?.value || day}`;
    ...
    c.sellUnit = 0; c.buyUnit = 0; c.qty = 0; c.svc = 0; c.disc = 0;
  ...
  new Set([...Object.keys(snap), ...Object.keys(c)]).forEach(k => {
    if (_histSkip.has(k)) return;
    const bv = snap[k] ?? '';
    const av = _histVal(c[k]);
...
function renderBepScenarioTable() {
  ...
  // As-is 열의 매출/매출원가는 월 선택으로만 채워지는 자동계산값이라 수동 입력 불가.
  // 판관비(sga)는 캠페인 데이터에서 끌어올 수 없는 회사 공통비용이라 As-is 열도 예외적으로 수동 입력 유지.
  const inputRow = (field) => cols.map((c, i) => {
    if (c.key === 'asis' && field !== 'sga') return `<td style="${tdV}">${_fmtBepWon(derived[i][field])}</td>`;
    if (!edit) return '';
  ...
  el.innerHTML = `<div style="overflow-x:auto;"><table class="bep-tbl" style="width:max-content;">
    ...
    <tbody>
      ...
      <tr><td style="${tdL}">영업이익</td>${derived.map(d => `<td style="${tdHiB}color:${d.opProfit>=0?'var(--red)':'#1971c2'};">${_fmtBepWon(d.opProfit)}</td>`).join('')}</tr>
...
async function toggleBepScenarioEdit() {
  ...
  if (_bepScenarioEdit) {
    ...
    try {
      ...
      _BEP_COLS.forEach(c => {
        if (c.key === 'asis') {
          // 매출/매출원가/기준월은 헤더의 월 선택으로만 채워짐(bepAsisMonthChange가 별도 저장) — 기존 값 유지.
          // 판관비만 캠페인 데이터로 못 끌어오는 값이라 여기서도 입력칸을 두고 같이 저장.
          data[c.key] = {
            ...(BEP_SCENARIO.asis || {}),
            sga: Math.round(+(document.getElementById('bep_sc_sga_asis')?.value) || 0),
          };
          return;
        }
        data[c.key] = {
...
function renderBepMonthlyTable() {
  ...
  el.innerHTML = `<div style="overflow-x:auto;"><table class="bep-tbl" style="width:max-content;">
    ...
    <tbody>
      ...
      <tr><td style="${tdL}">영업이익</td><td style="${tdHiB}background:#fff9e6;color:${sumOf('opProfit')>=0?'var(--red)':'#1971c2'};">${_fmtBepWon(sumOf('opProfit'))}</td>${rows.map(r => r.d ? `<td style="${tdHiB}color:${r.d.opProfit>=0?'var(--red)':'#1971c2'};">${_fmtBepWon(r.d.opProfit)}</td>` : `<td style="${tdDim}">—</td>`).join('')}</tr>
...
function renderStlPermCall(container) {
  ...
  data.forEach(c => {
    ...
    rows += `<tr onclick="openDetail(${idx})" style="cursor:pointer;">
      ...
      <td style="text-align:right;">${advU ? _fmtMoney(advU) + '원' : nd}</td>
      <td style="text-align:right;">${adc ? _fmtMoney(adc) + '원' : nd}</td>
      <td style="text-align:right;">${ohcCost ? _fmtMoney(ohcCost) + '원' : nd}</td>
      <td style="text-align:right;">${dnuCost ? _fmtMoney(dnuCost) + '원' : nd}</td>
      <td style="text-align:right;font-weight:700;color:${profit >= 0 ? 'var(--green)' : 'var(--red)'};">${profit ? _fmtMoney(profit) + '원' : nd}</td>
    ...
    <div class="settle-table-wrap">
      <table class="stl-table">
        ...
        <tfoot><tr style="font-weight:700;background:var(--surface2);">
          ...
          <td style="text-align:right;">${totAdc ? _fmtMoney(totAdc) + '원' : '—'}</td>
          <td style="text-align:right;">${totOhc ? _fmtMoney(totOhc) + '원' : '—'}</td>
          <td style="text-align:right;">${totDnu ? _fmtMoney(totDnu) + '원' : '—'}</td>
          <td style="text-align:right;color:${totPrf >= 0 ? 'var(--green)' : 'var(--red)'};">${totPrf ? _fmtMoney(totPrf) + '원' : '—'}</td>
...
function renderStlCpsView(container) {
  ...
  const pct = (n, d) => d > 0 ? (n / d * 100).toFixed(2) + '%' : nd;
  let totFinal = 0, totTotal = 0, totMedia = 0, totProfit = 0;
  const fmtMonth = d => { const p = (d || '').split('-'); return p.length >= 2 ? `${p[0]}년 ${+p[1]}월` : (d || '—'); };
  ...
  const rows = data.map(c => {
    ...
    return `<tr data-stlcamp="${c.id}" onclick="openDetail(${idx})" style="cursor:pointer;">
      ...
      <td style="text-align:right;font-weight:700;">${finalSales ? fmt(finalSales) + '원' : nd}</td>
      <td style="text-align:right;">${totalComm ? fmt(totalComm) + '원' : nd}</td>
      <td style="text-align:right;color:var(--text2);">${pct(totalComm, finalSales)}</td>
      <td style="text-align:right;">${mediaComm ? fmt(mediaComm) + '원' : nd}</td>
    ...
    <div class="settle-table-wrap">
      <table class="stl-table">
        ...
        <tfoot><tr style="font-weight:700;background:var(--surface2);">
          ...
          <td></td>
          <td style="text-align:right;">${totMedia ? fmt(totMedia) + '원' : '—'}</td>
          <td></td>
...
function renderStlGroupView(data, container, groupKey, groupLabel) {
  ...
  const rows = groupOrder.map(gname => {
    ...
    return `<tr data-stlgroup style="border-top:2px solid var(--border2);">
      ...
      <td class="td-num td-r" style="font-weight:700;">${tAdc ? fmt(Math.round(tAdc * 1.1)) : nd}</td>
      <td>${nd}</td>
      ...
      <td class="td-num td-r" style="font-weight:700;">${tBuy ? fmt(Math.round(tBuy * 1.1)) : nd}</td>
      <td>${nd}</td>
...
function _taxCanCorrectGroup(gid) {
  const items = TAX_DATA.filter(t => _taxGroupId(t) === gid);
  if (!items.length) return false;
  const rep = items.find(t => !t.isRef) || items[0];
  return !!(currentUser?.isAdmin || currentUser?.name === (rep.createdBy || rep.manager));
}
...
function renderTaxList() {
  ...
  // 수정발행으로 대체(취소)된 건 — 다른 그룹이 이 그룹을 correctionOf로 참조하면 이 그룹은 더 이상 유효하지 않음.
  // 최종금액=덮어쓰기 모델이라, 대체된 원본은 집계(합계/건수)에서 완전히 제외해야 이중계상이 안 됨.
  const _isSuperseded = gid => _correctionOfMap.has(gid);

  // 퀵필터 카운트 (필터 적용 전 전체 기준, 대체된 건 제외)
  const unissuedCnt   = groups.filter(([gid, items]) => !_isSuperseded(gid) && items[0].taxStatus !== '완료').length;
  const unpaidCnt     = groups.filter(([gid, items]) => !_isSuperseded(gid) && items[0].paid !== '완료').length;
  const correctionCnt = groups.filter(([, items]) => items[0].correctionOf != null).length;

  // 퀵필터 적용
  const displayGroups = _taxQuickFilter === 'unissued'
    ? groups.filter(([gid, items]) => !_isSuperseded(gid) && items[0].taxStatus !== '완료')
  ...
  const _v = n => n ? _fmtMoney(n) : '—';
  const _el = id => document.getElementById(id);
  const issuedCnt  = displayGroups.filter(([gid, items]) => !_isSuperseded(gid) && items[0].taxStatus === '완료').length;
  const paidCnt    = displayGroups.filter(([gid, items]) => !_isSuperseded(gid) && items[0].paid === '완료').length;
  ...
  const chipCorr = _el('tax-chip-correction');
  if (chipCorr) chipCorr.className = _taxQuickFilter === 'correction' ? 'btn btn-primary btn-sm' : 'btn btn-outline btn-sm';
  ...
  pageGroups.forEach(([gid, items]) => {
    ...
    const isSuperseded       = childCorrectionGid != null; // 수정발행으로 대체(취소)된 건 — 금액은 더 이상 유효하지 않음
    const isCorrection       = rep.correctionOf != null;
    const parentExists       = isCorrection && groupMap.has(rep.correctionOf);
    // backwardBadge(원본 표시) → [1,1] 번호 옆, forwardBadge(바로가기) → [1,2] 상태 줄. 서로 다른 위치라 둘 다 있어도 안 겹침.
    const backwardBadge = isCorrection
        ? (parentExists ? `<span class="tax-corr-link" onclick="event.stopPropagation();openTaxLinkedInfoModal(${rep.correctionOf})" style="cursor:pointer;font-size:10px;font-weight:600;color:#d97706;background:#fffbeb;border:1px solid #fde68a;padding:2px 7px;border-radius:10px;white-space:nowrap;">${_taxGroupLabel(rep.correctionOf)} 건의 수정발행</span>` : `<span style="font-size:10px;font-weight:600;color:var(--text3);background:var(--surface2);border:1px solid var(--border);padding:2px 7px;border-radius:10px;white-space:nowrap;">(삭제된 건)의 수정발행</span>`)
        : '';
    const forwardBadge = childCorrectionGid != null
        ? `<span class="tax-corr-link" onclick="event.stopPropagation();openTaxLinkedInfoModal(${childCorrectionGid})" style="cursor:pointer;font-size:10px;font-weight:600;color:#d97706;background:#fffbeb;border:1px solid #fde68a;padding:2px 7px;border-radius:10px;white-space:nowrap;">수정발행됨 → ${_taxGroupLabel(childCorrectionGid)} 바로가기</span>`
        : '';
    ...
    cards.push(`
      <div class="tax-card" data-gid="${gid}"${isSuperseded ? ' style="opacity:0.55;"' : ''}>
        <div class="tax-card-head">
          ...
          <div class="tax-card-head-main" style="display:grid;grid-template-columns:170px 1px 1fr;grid-template-rows:auto auto;gap:0;">
            ...
            <!-- [1,2]: 수정발행 바로가기·세발·입금·요청자·날짜들 (좌측 정렬) -->
            ...
            <div style="grid-column:3;grid-row:2;display:flex;align-items:center;gap:10px;">
              <span style="font-size:12px;color:var(--text2);${isSuperseded ? 'text-decoration:line-through;' : ''}">공급가 <b style="color:var(--text1);">${supplySum.toLocaleString()}</b></span>
              <span style="font-size:12px;color:var(--text2);${isSuperseded ? 'text-decoration:line-through;' : ''}">부가세포함 <b style="color:var(--text1);">${vatSum.toLocaleString()}</b></span>
              ${isSuperseded ? `<span style="font-size:11px;font-weight:600;color:var(--red);">취소됨(집계 제외)</span>` : ''}
              <span style="color:var(--border);">|</span>
...
// 원본↔수정발행 뱃지 클릭 시 — 리스트를 벗어나지 않고 연결된 건의 정보를 바로 확인
function openTaxLinkedInfoModal(gid) {
  const items = TAX_DATA.filter(t => _taxGroupId(t) === gid);
  if (!items.length) { toast('해당 건을 찾을 수 없습니다.', 'warn'); return; }
  const mainItems  = items.filter(t => !t.isRef).sort((a, b) => a.id - b.id);
  const dispItems  = mainItems.length ? mainItems : items.slice().sort((a, b) => a.id - b.id);
  const rep        = dispItems[0];
  const supplySum  = dispItems.reduce((s, t) => s + _taxRefSupplyAmt(t), 0);
  ...
  const rowsHtml = dispItems.map(t => `
    <tr>
  ...
  mEl.innerHTML = `<div class="modal" style="width:560px;max-width:94vw;">
    <div class="modal-head">
      <span class="modal-title">${_taxGroupLabel(gid)} 발행 정보</span>
      <button class="modal-close" onclick="closeModal('${mId}')">✕</button>
    </div>
    <div class="modal-body" style="display:flex;flex-direction:column;gap:12px;">
      <div style="display:flex;flex-wrap:wrap;gap:6px 20px;font-size:12px;color:var(--text2);">
        <span>업체명 <b style="color:var(--text1);">${_escHtml(rep.company || '—')}</b></span>
        <span>발행구분 <b style="color:var(--text1);">${rep.taxType === 'media' ? '매체' : '광고주'}</b></span>
        <span>담당자 <b style="color:var(--text1);">${_escHtml(rep.createdBy || rep.manager || '—')}</b></span>
        <span>요청일 <b style="color:var(--text1);">${rep.reqDate || '—'}</b></span>
        <span>발행일 <b style="color:var(--text1);">${rep.issueDate || '—'}</b></span>
        <span>세발 <b style="color:${isDone ? 'var(--green)' : 'var(--text1)'};">${isDone ? '완료' : '미완료'}</b></span>
        <span>입금 <b style="color:${isPaid ? 'var(--green)' : 'var(--text1)'};">${isPaid ? '완료' : '미완료'}</b></span>
      </div>
      <table style="width:100%;border-collapse:collapse;">
        <thead><tr>
          <th style="padding:5px 8px;border:1px solid var(--border);background:var(--surface2);font-size:11px;color:var(--text2);">집행월</th>
          <th style="padding:5px 8px;border:1px solid var(--border);background:var(--surface2);font-size:11px;color:var(--text2);">품목</th>
          <th style="padding:5px 8px;border:1px solid var(--border);background:var(--surface2);font-size:11px;color:var(--text2);">공급가액</th>
        </tr></thead>
        <tbody>${rowsHtml}</tbody>
      </table>
      <div style="text-align:right;font-size:12px;color:var(--text2);">공급가액 합계 <b style="color:var(--text1);">${supplySum.toLocaleString()}</b>원 · 부가세포함 <b style="color:var(--text1);">${vatSum.toLocaleString()}</b>원</div>
    </div>
    <div class="modal-foot">
      <button class="btn btn-ghost" onclick="closeModal('${mId}')">닫기</button>
      <button class="btn btn-primary" onclick="closeModal('${mId}');_taxScrollToGroup(${gid})">카드로 이동</button>
    </div>
  </div>`;
  openModal(mId);
}
...
async function deleteTax(id) {
  ...
  if (hasDependents && groupRowCount <= 1) {
    alert(`${_taxGroupLabel(gid)} 건을 원본으로 하는 수정발행 요청이 있어, 이 그룹의 마지막 남은 항목은 삭제할 수 없습니다.`);
    return;
  }
  if (!confirm(`${_taxGroupLabel(gid)} 항목을 삭제하시겠습니까?`)) return;
...
async function deleteGroup(gid) {
  ...
  if (dependentGids.size > 0) {
    alert(`${_taxGroupLabel(gid)} 건을 원본으로 하는 수정발행 요청이 있어 삭제할 수 없습니다. (연결된 수정발행: ${dependentGids.size}건)`);
    return;
  }
  if (!confirm(`${_taxGroupLabel(gid)} 발행요청 전체를 삭제하시겠습니까?`)) return;
...
function _taxRegRenderChips() {
  ...
  const rows = _taxRegLinkedCamps.map(cid => {
    const c = DATA.find(x => x.id === cid);
    if (!c) return '';
    const stl = _stlAmt(c);
    const supply = taxType === 'media' ? (stl.buyAmt || 0) : (stl.amt || 0);
    return `<tr>
    ...
    <table style="width:100%;border-collapse:collapse;border:1px solid var(--border);border-radius:6px;overflow:hidden;">
      <thead>
        <tr style="background:var(--surface2);">
          <th style="font-size:11px;font-weight:500;color:var(--text3);padding:5px 6px;text-align:left;width:80px;">집행월</th>
          <th style="font-size:11px;font-weight:500;color:var(--text3);padding:5px 6px;text-align:left;">상품</th>
          <th style="font-size:11px;font-weight:500;color:var(--text3);padding:5px 6px;text-align:left;">캠페인명</th>
          <th style="font-size:11px;font-weight:500;color:var(--text3);padding:5px 6px;text-align:right;width:100px;">공급가액</th>
          <th style="font-size:11px;font-weight:500;color:var(--text3);padding:5px 6px;text-align:left;">담당자</th>
...
async function saveTaxReg() {
  const _saveBtn = document.querySelector('#modalTaxReg .modal-foot .btn-primary');
  ...
  if (correctionOf) {
    if (!_taxCanCorrectGroup(correctionOf)) { toast('수정발행 등록 권한이 없습니다.', 'err'); return; }
    ...
    // 입력값은 차액이 아니라 최종금액 — 기존건은 취소(대체)되고 이 금액이 이 건의 새 확정금액이 됨
    const finalAmtChk    = rows.reduce((s, tr) => s + (parseFloat(tr.querySelector('.tax-r-row-supply')?.value) || 0), 0);
    const ok = confirm(
      `원본 ${_taxGroupLabel(correctionOf)} ${originItemsChk[0].company || ''} 건(${originAmtChk.toLocaleString()}원)을 취소하고,\n` +
      `최종금액 ${finalAmtChk.toLocaleString()}원으로 재발행하시겠습니까?`
    );
    if (!ok) return;
  }
  ...
  if (!isNew) {
    existingForEdit.forEach(t => _fbSaveTaxDeleteLog(t, '수정'));
  ...
  // TAX_DATA는 onSnapshot이 자동으로 갱신 — 수동 push 제거 (중복 방지)
  refItems.forEach(({ lc }) => {
  ...
  if (_taxM) _taxM.value = '';
  renderTaxList();
  } finally {
    if (_saveBtn) { _saveBtn.disabled = false; _saveBtn.textContent = '저장'; }
  }
}
...
function openTaxAutoGen() {
  ...
  _taxGenEligible = DATA.filter(c => {
    if (!_taxIsSettled(c)) return false;
    const mediaRec = MEDIA_DATA.find(m => m.company === c.media);
    const canAdv   = !c.taxAdvReq   && !_taxGenAdvSet.has(c.id);
    const canMedia = !c.taxMediaReq && !_taxGenMediaSet.has(c.id) && !(mediaRec?.taxYn === false);
    return canAdv || canMedia;
  });
  ...
  const fSearch = document.getElementById('tax-gen-fSearch');
  if (fSearch) fSearch.value = '';
  ['tax-gen-bulk-adv','tax-gen-bulk-media'].forEach(id => {
    const b = document.getElementById(id);
    if (b) { b.className = 'btn btn-ghost btn-sm'; b.style.fontSize = '11px'; b.style.padding = '3px 10px'; }
  });
...
function taxGenRenderList() {
  ...
  tbody.innerHTML = filtered.map(c => {
    ...
    const hasAdv   = c.taxAdvReq   || _taxGenAdvSet.has(c.id);
    const hasMedia = c.taxMediaReq || _taxGenMediaSet.has(c.id);
...
function taxGenBulkType(type) {
  ...
  taxGenUpdateTotal();
  const btnAdv   = document.getElementById('tax-gen-bulk-adv');
  const btnMedia = document.getElementById('tax-gen-bulk-media');
  if (btnAdv && btnMedia) {
    btnAdv.className   = 'btn btn-sm' + (type === 'adv'   ? ' btn-primary' : ' btn-ghost');
    btnMedia.className = 'btn btn-sm' + (type === 'media' ? ' btn-primary' : ' btn-ghost');
    btnAdv.style.fontSize = btnMedia.style.fontSize = '11px';
    btnAdv.style.padding  = btnMedia.style.padding  = '3px 10px';
  }
}
...
async function confirmTaxEdit() {
  ...
  const newItems = [];
  let nextNewId = _taxNextId(); // ID를 미리 확보해 순차 증가 (TAX_DATA push 없이도 중복 방지)
  ...
  // TAX_DATA는 onSnapshot이 자동으로 갱신 — 수동 조작 제거 (중복 방지)
  _taxEditGid = null;
...
async function confirmTaxAutoGen() {
  ...
  const _regBtn = document.querySelector('#tax-gen-step2 .modal-foot .btn-primary');
  if (_regBtn) { _regBtn.disabled = true; _regBtn.textContent = '등록 중…'; }
...
async function _fbSaveTax(t) {
  ...
  catch(e) { console.error('[FB] 세금계산서 저장 실패:', e); throw e; }
}
...
async function _fbSaveTaxDeleteLog(t, reason = '삭제') {
...
const _KPI_MONTHS = ['01','02','03','04','05','06','07','08','09','10','11','12'];
const _KPI_ML = {'01':'1월','02':'2월','03':'3월','04':'4월','05':'5월','06':'6월','07':'7월','08':'8월','09':'9월','10':'10월','11':'11월','12':'12월'};
const _KPI_QTR_COLS = ['01','02','03','Q1','04','05','06','Q2','07','08','09','Q3','10','11','12','Q4'];
const _KPI_QTR_MAP  = { Q1:['01','02','03'], Q2:['04','05','06'], Q3:['07','08','09'], Q4:['10','11','12'] };

function _fbWatchKpiTargets(year) {
  if (_kpiUnsub) { _kpiUnsub(); _kpiUnsub = null; }
  if (!window._db) return;
  _kpiUnsub = window._db.collection('settings').doc('kpi_targets_' + year)
    .onSnapshot(doc => {
      KPI_DATA = doc.exists ? doc.data() : {};
      if (document.getElementById('screen-kpi')?.classList.contains('active')) renderKpi();
    }, e => console.error('[FB] KPI 구독 실패:', e));
}

function _kpiCanEdit() {
...
function _kpiYoyHtml(actual, prev) {
  ...
  const r = Math.round((actual / prev - 1) * 100);
  if (r > 0)  return `<span class="kpi-up">▲${r}%</span>`;
  if (r < 0)  return `<span class="kpi-down">▼${Math.abs(r)}%</span>`;
  return `<span class="kpi-flat">±0%</span>`;
}
...
function initKpiScreen() {
  ...
  if (orgSel) {
    orgSel.innerHTML = '<option value="">전사 전체</option>' + _buildOrgSelectHTML();
...
function kpiOrgChange() {
  _kpiOrgFilter = document.getElementById('kpi-org-filter')?.value || '';
...
function renderKpiForecastBanner() {
  ...
  const curM    = String(new Date().getMonth() + 1).padStart(2, '0');
  const passed  = _KPI_MONTHS.filter(m => m <= curM);
  const remain  = 12 - passed.length;
  const cumActual = passed.reduce((s, m) => s + _kpiCalcActual(_kpiYear, '', '', m), 0);
  if (!passed.length || !cumActual) { el.innerHTML = ''; return; }
  const pace     = cumActual / passed.length;
  const forecast = Math.round(cumActual + pace * remain);
  const fcPct    = annualTarget ? Math.round(forecast / annualTarget * 100) : 0;
  const gap      = annualTarget - forecast;
  const gapColor = gap <= 0 ? 'var(--green)' : 'var(--red)';
  const gapLabel = gap <= 0 ? `목표 초과 ${_fmtW(Math.abs(gap))}` : `잔여 ${_fmtW(gap)}`;
  el.innerHTML = `<div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:11px 18px;font-size:13px;color:var(--text2);display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
    <span style="color:var(--text3);font-size:12px;">현재 페이스 기준 연말 예상 실적</span>
    <strong style="font-size:15px;color:var(--text1);">${_fmtW(forecast)}</strong>
    <span style="color:var(--text3);">·</span>
    <span>목표 <strong>${_fmtW(annualTarget)}</strong> 대비 <strong style="color:${fcPct>=100?'var(--green)':'var(--accent)'};">${fcPct}%</strong></span>
    <span style="color:var(--text3);">·</span>
    <span style="color:${gapColor};font-weight:600;">${gapLabel}</span>
    <span style="color:var(--text3);">·</span>
    <span style="color:var(--text3);font-size:12px;">남은 기간 ${remain}개월</span>
  </div>`;
}

function renderKpiGrandTable() {
  const el = document.getElementById('kpi-grand-table');
  if (!el) return;
  ...
  const totAct  = _KPI_MONTHS.reduce((s, m) => s + act[m], 0);
...
function renderKpiClientListTable() {
  ...
  _kpiValidBonbus().forEach(b => {
    ...
    (b.teams || []).forEach(t => {
      ...
      teamsToShow.push({ bonbuName: b.name, ...t });
    });
  });
...
function renderKpiOrgTable(targetId, orgFilterOverride) {
  ...
  // 파랗게 구분한다 — td마다 이미 인라인으로 1px 테두리를 박아놔서(border:1px solid var(--border))
  // !important 없이는 안 먹는다.
  let html = `<style>
    .kpi-bonbu-sep td{border-top:2px solid var(--border) !important;}
    .kpi-bonbu-end-sep td{border-top:2px solid var(--accent) !important;}
...
async function saveKpiTargets() {
  ...
  if (!window._db) { toast('DB 연결 없음', 'err'); return; }
  const btn = document.getElementById('ki_save_btn');
  if (btn) { btn.disabled = true; btn.textContent = '저장 중…'; }
  try {
...
function renderPerfMatrix() {
  ...
  if (!data.length) {
    container.innerHTML = '<div style="padding:48px;text-align:center;color:var(--text3);font-size:13px;">해당 조건의 CTR 데이터가 없습니다.</div>';
    return;
  }
...
function perfShowTooltip(e, el) {
  ...
  tip.innerHTML = `
    ...
    <div style="color:var(--text2);">캠페인 &nbsp;<b style="color:var(--text1);">${el.dataset.count}건</b></div>`;
  tip.style.display = 'block';
  tip.style.left = (e.clientX + 14) + 'px';
  tip.style.top  = (e.clientY - 10) + 'px';
}

function perfHideTooltip() {
  const tip = document.getElementById('perf-tooltip');
...
function renderPerfHistory() {
  ...
  const advMap = {};
  for (const c of data) {
    const adv = c.seller || c.adv || '(매출처 없음)';
